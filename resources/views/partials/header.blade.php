@auth
    @if (is_role() == 'super admin' || is_role() == 'admin')
                ...
                <div class="navbar-collapse justify-content-end px-0" id="navbarNav">
                    <ul class="navbar-nav flex-row ms-auto align-items-center justify-content-end">
                        ...
                        @if (session('super_admin') && !empty(session('super_admin')))
                            <li class="nav-item">
                                <a href="{{ route('backtoadmin') }}?admin=1"
                                    class="btn btn-outline-danger mx-3 mt-2 d-block">Back to super admin </a>
                            </li>
                        @endif
                        @if (session('company_admin') && !empty(session('company_admin')))
                            <li class="nav-item">
                                <a href="{{ route('backtoadmin') }}?company=1"
                                    class="btn btn-outline-primary mx-3 mt-2 d-block">Login as agency </a>
                            </li>
                        @endif
                        <li class="nav-item dropdown">
                            <a class="nav-link nav-icon-hover" href="javascript:void(0)" id="drop2"
                                data-bs-toggle="dropdown" aria-expanded="false">
                                <img src=" {{ asset(auth()->user()->image ?? '/assets/images/profile/user-1.jpg') }}"
